...
var sfxlist=[
	...
	{name:"darkness",vm:3},
	{name:"darknesse",vm:3},
	{name:"indestructible",vm:2.5},
	{name:"indestructibleh",vm:2.5},
	{name:"obliteratem",vm:1.3},
	{name:"obliterates",vm:1.3},
	{name:"obliterate1",vm:1.3},
	{name:"obliterate2",vm:1.3},
	{name:"obliterate3",vm:1.3},
	{name:"beastdeath",vm:0.45},
	{name:"vinetrap",vm:2.5},
];
var loadassetscache=function(){
	...
	text("0/3",500,370);
		...
		sprites={
			...
			keystones:new Array(),
			properties:{
				fire:loadShape('Data/Graphics/properties/fire.svg'),
				water:loadShape('Data/Graphics/properties/water.svg'),
				earth:loadShape('Data/Graphics/properties/earth.svg'),
				electric:loadShape('Data/Graphics/properties/electric.svg'),
			},
		};
		for(gc=0;gc<keystones.length;gc+=1){
			append(sprites.keystones,loadShape('Data/Graphics/keystones/'+gc+'.svg'));
		}
		fill(0,0,0);
		rect(400,350,300,100);
		fill(255,150,0);
		...
		sfx={
				...
				crit: new Howl({src: ['Data/Sound/sfx/crit.ogg'], autoplay: false, loop: false,volume: options.sfx*0.2,}),
				fissurec: new Howl({src: ['Data/Sound/sfx/fissurec.ogg'], autoplay: false, loop: false,volume: options.sfx*2,}),
				fissure: new Howl({src: ['Data/Sound/sfx/fissure.ogg'], autoplay: false, loop: false,volume: options.sfx*2.5,}),
				...
				darkness:new Howl({src: ['Data/Sound/sfx/darkness.ogg'], autoplay: false,loop: true,volume: options.sfx*3,}),
				darknesse:new Howl({src: ['Data/Sound/sfx/darknesse.ogg'], autoplay: false,loop: false,volume: options.sfx*3,}),
				indestructible:new Howl({src: ['Data/Sound/sfx/indestructible.ogg'], autoplay: false,loop: false,volume: options.sfx*2.5,}),
				indestructibleh:new Howl({src: ['Data/Sound/sfx/indestructibleh.ogg'], autoplay: false,loop: false,volume: options.sfx*2.5,}),
				obliteratem:new Howl({src: ['Data/Sound/sfx/obliteratem.ogg'], autoplay: false,loop: false,volume: options.sfx*1.3,}),
				obliterates:new Howl({src: ['Data/Sound/sfx/obliterates.ogg'], autoplay: false,loop: false,volume: options.sfx*1.3,}),
				obliterate1:new Howl({src: ['Data/Sound/sfx/obliterate1.ogg'], autoplay: false,loop: false,volume: options.sfx*1.3,}),
				obliterate2:new Howl({src: ['Data/Sound/sfx/obliterate2.ogg'], autoplay: false,loop: false,volume: options.sfx*1.3,}),
				obliterate3:new Howl({src: ['Data/Sound/sfx/obliterate3.ogg'], autoplay: false,loop: false,volume: options.sfx*1.3,}),
				beastdeath:new Howl({src: ['Data/Sound/sfx/thatbeastdeath.ogg'], autoplay: false,loop: false,volume: options.sfx*0.45,}),
				vinetrap:new Howl({src: ['Data/Sound/sfx/vines.ogg'], autoplay: false,loop: false,volume: options.sfx*2.5,}),
		};
		}
		loadassetscache=0;
		canstart=1;
	...
	if(enemies[i].prefix){
		...
		if(eprefixdata[(enemies[i].prefix-1)*5+4]){
			for(aea=0;aea<eprefixdata[(enemies[i].prefix-1)*5+4].length;aea+=1){
				...
				else if(eprefixdata[(enemies[i].prefix-1)*5+4][aea][0]==13){
					append(enemies[i].traits.damagedealtpm,applyNMETrait[eprefixdata[(enemies[i].prefix-1)*5+4][aea][1]](eprefixdata[(enemies[i].prefix-1)*5+4][aea][2]));
				}
			}
		}
	}
	if(enemies[i].suffix){
		...
		if(esuffixdata[(enemies[i].suffix-1)*5+4]){
			for(aea=0;aea<esuffixdata[(enemies[i].suffix-1)*5+4].length;aea+=1){
				...
				else if(esuffixdata[(enemies[i].suffix-1)*5+4][aea][0]==13){
					append(enemies[i].traits.damagedealtpm,applyNMETrait[esuffixdata[(enemies[i].suffix-1)*5+4][aea][1]](esuffixdata[(enemies[i].suffix-1)*5+4][aea][2]));
				}
			}
		}
	}
...
var showkeystonetree=function(ctree){
	...
	for(kt=1;kt<keystonecache[ctree].length;kt+=1){
		for(sk=0;sk<keystonecache[ctree][kt].length;sk+=1){
			...
			shape(sprites.keystones[keystonecache[ctree][kt][sk].id],400+(sk+1)*700/(1+keystonecache[ctree][kt].length),kt*100,350,468);
			for(dkp=0;dkp<player.keystones.keystones[keystonecache[ctree][kt][sk].id]+keystonecache.simulated.keystones[keystonecache[ctree][kt][sk].id];dkp+=1){
				if(dkp<player.keystones.keystones[keystonecache[ctree][kt][sk].id]){
					fill(50,255,50);
				}
				else{
					fill(255,255,255);
				}
				ellipse(sin(dkp*PI*2/keystonecache[ctree][kt][sk].max)*42+396+(sk+1)*700/(1+keystonecache[ctree][kt].length),cos(dkp*PI*2/keystonecache[ctree][kt][sk].max)*42+kt*100-4,8,8);
			}
		...
		if(inventory==1){
			if(inventype==1){
				...
				helpscreen.help=function(){
					...
					fill(255,255,255);
					textFont(txtfont,30);
					text("Space-time Manipulator Help",100,0,933,50);
					
					textFont(txtfont,22);
					fill(255,255,255,200);
					text("Click on a rift to enter it. The rift on the left is free while the one on the right costs reactant. If a different level is desired, type it in (highest selectable rift level is 5 more than the highest level rift you have cleared. Also note that rift levels are rounded to the nearest multiple of 5).",33,100,1100,600);
...
function createenemy(ID){
	...
	if(enemydata[ID*30+28]){
		for(aea=0;aea<enemydata[ID*30+28].length;aea+=1){
			...
			else if(enemydata[ID*30+28][aea][0]==13){
				append(this.traits.damagedealtpm,applyNMETrait[enemydata[ID*30+28][aea][1]](enemydata[ID*30+28][aea][2]));
			}
		}
	}
...
var applyNMETrait=[
	...
	//21: spirit guard
		function(args){
			...
			return(
				function(){
					if(enemies[index].ts.spiritguard){
						if(willhit){
							pdmg=0;
							mdmg=0;
							willhit=0;
							append(dmgind,new cdmgind(enemies[index].x,enemies[index].y-5,"Blocked",15,140,90,140));
							enemies[index].ts.spiritguard=0;
							sfx.ksblock.rate(random(0.9,1.1));
							sfx.ksblock.volume(options.sfx*1.4);
							sfx.ksblock.play();
						}
						else{
							if(armorE>0){
	...
	//22: spirit guard visual
		function(args){
			return(
				function(){
					if(enemies[i].ts.spiritguard){
						fill(160,50,160,80+abs(tick%90-45)*1.7);
						ellipseMode(CENTER);
						ellipse(enemies[i].x-playertemp.x+400,enemies[i].y-playertemp.y+350,20+enemies[i].size*2,20+enemies[i].size*2);
					}
				}
			);
		},
	//23: spirit guard setup
		function(args){
			return(
				function(){
					if(!(enemies[i].ts.spiritguardsetup)){
						enemies[i].ts.spiritguardsetup=1;
						enemies[i].ts.spiritguard=1;
					}
				}
			);
		},
	//24: spirit guard recharge
		function(args){
			args=split(args,"/");
			for(sta=0;sta<args.length;sta+=1){
				args[sta]=Number(args[sta]);
			...
			return(
				function(){
					if(willhit){
						if(!(enemies[attacker].ts.spiritguardrec)){
							enemies[attacker].ts.spiritguardrec=0;
						}
						enemies[attacker].ts.spiritguardrec+=1;
						if(enemies[attacker].ts.spiritguardrec>=args[0]){
							enemies[attacker].ts.spiritguardrec=0;
							enemies[attacker].ts.spiritguard=1;
						}
					}
				}
			);
			...
			if(cursorbox(x,x+50,y,y+50)){
				...
				if(!(mouselock)&mousePressed&mouseButton==RIGHT){
					...
					if(playertemp.inBossFight){
						append(particles,new createparticle(mouseX,mouseY-30,0,0,0,0,'text','You cannot unequip items in boss arenas!',20,0,255,-3,255,0,0));
					}
					else{
						if(findemptyslot()>-1){
							if(options.loadAudio){sfx.click3.play();}
...
var getloot=function(f){
	...
	if(player.inrift){
		player.riftsouls+=enemies[f].soulv;
		if(player.riftsouls>400&player.riftlv<biomedata[9]){
			append(particles,new createparticle(250,300,0,-0.7,0,0,'text','Rift Complete!',30,0,255,-1.5,190,240,0));
			player.riftlv=biomedata[9];
		}
	}
...
var applykeystones=function(){
	...
	if(player.passiveboost){
		player.passivemults.power.hp+=player.passiveboost;
		player.passivemults.power.hpregen+=player.passiveboost;
		player.passivemults.power.str+=player.passiveboost;
		player.passivemults.power.intel+=player.passiveboost;
		player.passivemults.power.armor+=player.passiveboost;
		player.passivemults.power.res+=player.passiveboost;
		player.passivemults.fortitude.hp+=player.passiveboost;
		player.passivemults.fortitude.hpregen+=player.passiveboost;
		player.passivemults.fortitude.str+=player.passiveboost;
		player.passivemults.fortitude.intel+=player.passiveboost;
		player.passivemults.fortitude.armor+=player.passiveboost;
		player.passivemults.fortitude.res+=player.passiveboost;
		player.passivemults.omnipotency.hp+=2*player.passiveboost;
		player.passivemults.omnipotency.hpregen+=2*player.passiveboost;
		player.passivemults.omnipotency.str+=2*player.passiveboost;
		player.passivemults.omnipotency.intel+=2*player.passiveboost;
		player.passivemults.omnipotency.armor+=2*player.passiveboost;
		player.passivemults.omnipotency.res+=2*player.passiveboost;
	}
	recalstats();
	loadkeystoneps();
}
...
var applydrench=function(target,power,duration){
	if(!(enemies[target].drenchcount)){
		enemies[target].drenchcount=0;
	}
	enemies[target].drenchcount+=power;
	append(enemies[target].tstatus,{name:'drench',tick:0,dir:random(PI*2),run:function(){
		enemies[i].tstatus[d].tick+=1;
		if((tick)%(round(60/power))==0){
		...
		if(enemies[i].tstatus[d].tick>=duration){
			enemies[i].drenchcount-=power;
			enemies[i].tstatus.splice(d,1);
			d-=1;
		}
	}
...
var applyunbalance=function(target,power,duration){
	if(!(enemies[target].unbalancecount)){
		enemies[target].unbalancecount=0;
	}
	enemies[target].unbalancecount+=power;
	append(enemies[target].tstatus,{name:'unbalance',tick:0,dir:random(PI*2),run:function(){
		enemies[i].tstatus[d].tick+=1;
		if((tick)%(round(60/power))==0){
			append(particles,new createparticle(enemies[i].x+random(-enemies[i].size,enemies[i].size),enemies[i].y+enemies[i].size,random(-0.5,0.5),0,0,random(0.03,0.07),'circle','',12,-0.2,255,-9,random(100,150),random(80,120),random(40),1));
		}
		if(enemies[i].tstatus[d].tick>=duration){
			enemies[i].unbalancecount-=power;
			enemies[i].tstatus.splice(d,1);
			d-=1;
		}
	}
...
var applyshock=function(target,power,duration){
	if(!(enemies[target].shockcount)){
		enemies[target].shockcount=0;
	}
	enemies[target].shockcount+=power;
	append(enemies[target].tstatus,{name:'shock',tick:0,dir:random(PI*2),run:function(){
		enemies[i].tstatus[d].tick+=1;
		if((tick)%(round(40/power))==0){
			append(particles,new createparticle(enemies[i].x+random(-enemies[i].size*2,enemies[i].size*2),enemies[i].y+random(-enemies[i].size*2,enemies[i].size*2),0,0,0,0,'circle','',8,-0.3,255,-30,random(220,255),random(230,255),random(100,150),1));
			append(particles,new createparticle(enemies[i].x+random(-enemies[i].size*2,enemies[i].size*2),enemies[i].y+random(-enemies[i].size*2,enemies[i].size*2),0,0,0,0,'circle','',8,-0.3,255,-30,random(100,150),random(100,150),random(200,255),1));
		}
		if(enemies[i].tstatus[d].tick>=duration){
			enemies[i].shockcount-=power;
			enemies[i].tstatus.splice(d,1);
			d-=1;
		}
	}
...
var applyarcaneinstability=function(target,power,duration){
	if(!(enemies[target].arcaneinstability)){
		enemies[target].arcaneinstability=0;
	}
	enemies[target].arcaneinstability+=power;
	append(enemies[target].tstatus,{name:'arcane instability',tick:0,dir:random(PI*2),run:function(){
		enemies[i].tstatus[d].tick+=1;
		if((tick)%(round(6/power))==0){
			append(particles,new createparticle(enemies[i].x+random(-enemies[i].size,enemies[i].size),enemies[i].y+random(-enemies[i].size,enemies[i].size),random(-0.5,0.5),0,0,random(-0.07,-0.03),'circle','',12,-0.2,255,-9,120+random(75),random(30),120+random(75),1));
		}
		if(enemies[i].tstatus[d].tick>=duration){
			enemies[i].arcaneinstability-=power;
			enemies[i].tstatus.splice(d,1);
			d-=1;
		}
	}
...
var loadkeystoneps=function(){
	...
	if(playertemp.keystonepassives[33]>0){
		append(keystonefuncs.damagedealtpostmit,function(){
			if(!(playertemp.sacrificialwella>0)){
				if(pdmg>0){
					playertemp.sacrificialwell=min((plshp(0.2)+plshr(4)+plsst(0.8)+plsin(0.8))*playertemp.keystonepassives[33],playertemp.sacrificialwell+pdmg*playertemp.keystonepassives[33]/100);
				}
				if(mdmg>0){
					playertemp.sacrificialwell=min((plshp(0.2)+plshr(4)+plsst(0.8)+plsin(0.8))*playertemp.keystonepassives[33],playertemp.sacrificialwell+mdmg*playertemp.keystonepassives[33]/100);
				}
			}
		});
		append(keystonefuncs.overlay,function(){
			fill(50,0,0,150);
			rect(760,400,40,playertemp.keystonepassives[33]*15);
			fill(255,0,0,150);
			rect(760,400+playertemp.keystonepassives[33]*15-(playertemp.sacrificialwell/(plshp(0.2)+plshr(4)+plsst(0.8)+plsin(0.8)))*15,40,(playertemp.sacrificialwell/(plshp(0.2)+plshr(4)+plsst(0.8)+plsin(0.8)))*15);
		});
		append(keystonefuncs.passives,function(){
			if(!(playertemp.sacrificialwell)){
				playertemp.sacrificialwell=0;
			}
			if(playertemp.sacrificialwell>=(plshp(0.2)+plshr(4)+plsst(0.8)+plsin(0.8))*playertemp.keystonepassives[33]){
				playertemp.sacrificialwella=1;
			}
			else if(playertemp.sacrificialwell<=0){
				playertemp.sacrificialwella=0;
			}
			if(playertemp.sacrificialwella>0){
				append(particles,new createparticle(780+random(-17,17),400+playertemp.keystonepassives[33]*15,0,0,0,random(-0.1,-0.3),'circle','',10,-0.2,255,random(-5,-10),255,random(160),0));
			}
			else{
				if(player.hp>plshp(0.8)){
					playertemp.sacrificialwell=min((plshp(0.2)+plshr(4)+plsst(0.8)+plsin(0.8))*playertemp.keystonepassives[33],playertemp.sacrificialwell+(player.hp*(0.02+playertemp.keystonepassives[33]*0.003))*playertemp.keystonepassives[33]/60);
					player.hp*=1-playertemp.keystonepassives[33]/3000;
				}
			}
		...
		append(keystonefuncs.dmgdealtpremod,function(){
			if(playertemp.sacrificialwella>0){
				if(playertemp.sacrificialwell>0){
					if(pdmg>mdmg){
						if(willhit){
							pdmg+=min(playertemp.sacrificialwell,(plshp(0.005)+plshr(0.1)+plsst(0.02)+plsin(0.02))+playertemp.sacrificialwell/20);
							playertemp.sacrificialwell-=min(playertemp.sacrificialwell,(plshp(0.005)+plshr(0.1)+plsst(0.02)+plsin(0.02))+playertemp.sacrificialwell/20);
						}
						else{
							pdmg+=min(playertemp.sacrificialwell,((plshp(0.005)+plshr(0.1)+plsst(0.02)+plsin(0.02))+playertemp.sacrificialwell/10)/60);
							playertemp.sacrificialwell-=min(playertemp.sacrificialwell,((plshp(0.005)+plshr(0.1)+plsst(0.02)+plsin(0.02))+playertemp.sacrificialwell/20)/60);
						}
					}
					else{
						if(willhit){
							mdmg+=min(playertemp.sacrificialwell,(plshp(0.005)+plshr(0.1)+plsst(0.02)+plsin(0.02))+playertemp.sacrificialwell/10);
							playertemp.sacrificialwell-=min(playertemp.sacrificialwell,(plshp(0.005)+plshr(0.1)+plsst(0.02)+plsin(0.02))+playertemp.sacrificialwell/20);
						}
						else{
							mdmg+=min(playertemp.sacrificialwell,((plshp(0.005)+plshr(0.1)+plsst(0.02)+plsin(0.02))+playertemp.sacrificialwell/10)/60);
							playertemp.sacrificialwell-=min(playertemp.sacrificialwell,((plshp(0.005)+plshr(0.1)+plsst(0.02)+plsin(0.02))+playertemp.sacrificialwell/20)/60);
						}
					}
				}
	...
	if(playertemp.keystonepassives[35]>0){
		append(keystonefuncs.overlay,function(){
			if(playertemp.fireboona>0){
				shape(sprites.properties.fire,820,470,40,40);
				append(particles,new createparticle(820+random(-10,10),486,random(-0.4,0.4),0,0,random(-0.1,-0.3),'circle','',8,-0.2,255,random(-5,-10),255,random(160),0));
			}
		});
		append(keystonefuncs.passives,function(){
			if(!(playertemp.fireboonc)){
				playertemp.fireboonc=0;
			}
			if(!(playertemp.fireboona)){
				playertemp.fireboona=0;
			}
			if(!(playertemp.fireboong)){
				playertemp.fireboong=random(300,1200);
			}
			if(playertemp.fireboona>0){
				playertemp.fireboona-=1;
			}
			else{
				playertemp.fireboonc+=1;
				if(playertemp.fireboonc>=playertemp.fireboong){
					playertemp.fireboona=360;
					playertemp.fireboonc-=780;
					playertemp.fireboong=random(300,1200);
				}
			}
		});
		...
		append(keystonefuncs.damagedealt,function(){
			if(playertemp.fireboona>0){
				if(willhit){
					pdmg*=1+playertemp.keystonepassives[35]*0.01;
					mdmg*=1+playertemp.keystonepassives[35]*0.01;
				}
				else{
					pdmg*=1+playertemp.keystonepassives[35]*0.03;
					mdmg*=1+playertemp.keystonepassives[35]*0.03;
				}
			}
		});
		append(keystonefuncs.damagedealtpostmit,function(){
			if(playertemp.fireboona>0){
				if(willhit){
					if(random(1)<=playertemp.keystonepassives[35]*0.05){
						...
						append(enemies[index].dots,{
							...
							dur:180,
							ptchan:round(random(19)),
							run:function(i){
								if(options.light){
									if((tick+enemies[i].dots[d].ptchan)%20==0){
	...
	if(playertemp.keystonepassives[36]>0){
		append(keystonefuncs.overlay,function(){
			if(playertemp.aquaboona>0){
				shape(sprites.properties.water,1100,470,30,45);
				append(particles,new createparticle(1100+random(-10,10),450+random(30),random(-0.2,0.2),0,0,random(0.15,0.2),'circle','',8,-0.2,255,random(-5,-10),0,0,random(200,255)));
			}
		});
		append(keystonefuncs.passives,function(){
			if(!(playertemp.aquaboonc)){
				playertemp.aquaboonc=0;
			}
			if(!(playertemp.aquaboona)){
				playertemp.aquaboona=0;
			}
			if(!(playertemp.aquaboong)){
				playertemp.aquaboong=random(300,1200);
			}
			if(playertemp.aquaboona>0){
				playertemp.aquaboona-=1;
			}
			else{
				playertemp.aquaboonc+=1;
				if(playertemp.aquaboonc>=playertemp.aquaboong){
					playertemp.aquaboona=360;
					playertemp.aquaboonc-=780;
					playertemp.aquaboong=random(300,1200);
				}
			}
		});
		...
		append(keystonefuncs.damagedealt,function(){
			if(playertemp.aquaboona>0){
				if(willhit){
					pdmg*=1+playertemp.keystonepassives[36]*0.01;
					mdmg*=1+playertemp.keystonepassives[36]*0.01;
				}
				else{
					pdmg*=1+playertemp.keystonepassives[36]*0.03;
					mdmg*=1+playertemp.keystonepassives[36]*0.03;
				}
			}
		});
		append(keystonefuncs.damagedealtpostmit,function(){
			if(playertemp.aquaboona>0){
				if(willhit){
					if(random(1)<=playertemp.keystonepassives[36]*0.1*procc){
						applydrench(index,10,120);
					}
				}
			}
		});
	}
	if(playertemp.keystonepassives[37]>0){
		append(keystonefuncs.overlay,function(){
			if(playertemp.earthenboona>0){
				shape(sprites.properties.earth,820,520,25,38);
				append(particles,new createparticle(820+random(-15,15),535,0,0,0,random(0.05,0.2),'circle','',8,-0.2,255,random(-5,-10),random(40,100),random(30,100),random(40)));
			}
		});
		append(keystonefuncs.passives,function(){
			if(!(playertemp.earthenboonc)){
				playertemp.earthenboonc=0;
			}
			if(!(playertemp.earthenboona)){
				playertemp.earthenboona=0;
			}
			if(!(playertemp.earthenboong)){
				playertemp.earthenboong=random(300,1200);
			}
			if(playertemp.earthenboona>0){
				playertemp.earthenboona-=1;
			}
			else{
				playertemp.earthenboonc+=1;
				if(playertemp.earthenboonc>=playertemp.earthenboong){
					playertemp.earthenboona=360;
					playertemp.earthenboonc-=780;
					playertemp.earthenboong=random(300,1200);
				}
			}
		});
		...
		append(keystonefuncs.damagedealt,function(){
			if(playertemp.earthenboona>0){
				if(willhit){
					pdmg*=1+playertemp.keystonepassives[37]*0.01;
					mdmg*=1+playertemp.keystonepassives[37]*0.01;
				}
				else{
					pdmg*=1+playertemp.keystonepassives[37]*0.03;
					mdmg*=1+playertemp.keystonepassives[37]*0.03;
				}
			}
		});
		append(keystonefuncs.damagedealtpostmit,function(){
			if(playertemp.earthenboona>0){
				if(willhit){
					if(random(1)<=playertemp.keystonepassives[37]*0.1*procc){
						applyunbalance(index,10,120);
					}
				}
			}
		});
	}
	if(playertemp.keystonepassives[38]>0){
		append(keystonefuncs.overlay,function(){
			if(playertemp.chargedboona>0){
				shape(sprites.properties.electric,1100,520,30,45);
				append(particles,new createparticle(1100+random(-10,10),520+random(-20,20),random(-0.2,0.2),0,0,0,'circle','',8,-0.2,255,random(-15,-25),random(200,255),random(200,255),random(50,180)));
				append(particles,new createparticle(1100+random(-10,10),520+random(-20,20),random(-0.2,0.2),0,0,0,'circle','',8,-0.2,255,random(-15,-25),random(200,255),random(200,255),random(50,180)));
			}
		});
		append(keystonefuncs.passives,function(){
			if(!(playertemp.chargedboonc)){
				playertemp.chargedboonc=0;
			}
			if(!(playertemp.chargedboona)){
				playertemp.chargedboona=0;
			}
			if(!(playertemp.chargedboong)){
				playertemp.chargedboong=random(300,1200);
			}
			if(playertemp.chargedboona>0){
				playertemp.chargedboona-=1;
			}
			else{
				playertemp.chargedboonc+=1;
				if(playertemp.chargedboonc>=playertemp.chargedboong){
					playertemp.chargedboona=360;
					playertemp.chargedboonc-=780;
					playertemp.chargedboong=random(300,1200);
				}
			}
		});
		...
		append(keystonefuncs.damagedealt,function(){
			if(playertemp.chargedboona>0){
				if(willhit){
					pdmg*=1+playertemp.keystonepassives[38]*0.01;
					mdmg*=1+playertemp.keystonepassives[38]*0.01;
				}
				else{
					pdmg*=1+playertemp.keystonepassives[38]*0.03;
					mdmg*=1+playertemp.keystonepassives[38]*0.03;
				}
			}
		});
		append(keystonefuncs.damagedealtpostmit,function(){
			if(playertemp.chargedboona>0){
				if(willhit){
					if(random(1)<=playertemp.keystonepassives[38]*0.1*procc){
						applyshock(index,10,120);
					}
				}
			}
		});
	...
	if(playertemp.keystonepassives[21]>0){
		append(keystonefuncs.damagetaken,function(){
			pdmg*=1-playertemp.keystonepassives[21]*0.01;
			mdmg*=1-playertemp.keystonepassives[21]*0.01;
		});
		append(keystonefuncs.dmgpremod,function(){
			if(playertemp.omnishell>0){
				armorE*=1+playertemp.keystonepassives[21]*0.1;
				resE*=1+playertemp.keystonepassives[21]*0.1;
			}
		});
	}
	...
	if(playertemp.keystonepassives[25]>0){
		...
		append(keystonefuncs.damagetaken,function(){
			if(attacktype=="DoT"){
				if(player.hp<plshp(0.1)){
					pdmg*=1-playertemp.keystonepassives[25]*0.09;
					mdmg*=1-playertemp.keystonepassives[25]*0.09;
				}
				else{
					pdmg*=1-playertemp.keystonepassives[25]*0.01;
					mdmg*=1-playertemp.keystonepassives[25]*0.01;
				}
			}
		...
		append(keystonefuncs.passives,function(){
			...
			while(playertemp.shockwavecharges>=5){
				...
				append(objects,{
					...
					x:playertemp.x,
					y:playertemp.y,
					pdmgmin:(plshp(0.4)+plsar(0.4)+plsre(0.4))*playertemp.keystonepassives[32],
					pdmgmax:(plshp(0.4)+plsar(0.4)+plsre(0.4))*playertemp.keystonepassives[32],
					mdmgmin:0,
					mdmgmax:0,
					armorE:1,
					resE:1,
					procc:0.08*playertemp.keystonepassives[32],
					run:function(){
						objects[n].size+=12;
			...
			if(playertemp.aegis&willhit){
				...
				if(options.loadAudio){
					if(sfxstock.ksblock>0){
						...
						if(pdmg+mdmg>0){
							sfx.ksblock.rate(random(0.9,1.1));
							sfx.ksblock.volume(options.sfx*1.6);
							sfx.ksblock.play();
						}
						else{
							sfx.ksblock.rate(random(0.9,1.1));
	...
	if(playertemp.keystonepassives[34]>0){
		append(keystonefuncs.healthextensions,function(){
			if(pdmg+mdmg>0){
				if(playertemp.elementalward>0){
					if(findprop("fire")||findprop("water")||findprop("earth")||findprop("nature")||findprop("electric")||findprop("cold")){
						if(willhit){
							append(particles,new createparticle(775,295,random(-2.2,-1.5),0,0,random(-0.08,0.08),'text',"-"+round(min(pdmg+mdmg,playertemp.elementalward)),22,0,255,-4,abs(tick%180-90)*255/90,abs(tick%136-68)*255/68,abs(tick%334-167)*255/167));
						}
						if(pdmg>playertemp.elementalward){
							pdmg-=playertemp.elementalward;
							playertemp.elementalward=0;
						}
						else{
							playertemp.elementalward-=pdmg;
							pdmg=0;
						}
						if(mdmg>playertemp.elementalward){
							mdmg-=playertemp.elementalward;
							playertemp.elementalward=0;
						}
						else{
							playertemp.elementalward-=mdmg;
							mdmg=0;
						}
					}
				}
		...
		append(keystonefuncs.passives,function(){
			...
			playertemp.elementalward=min(player.maxhp*0.2*playertemp.keystonepassives[34],playertemp.elementalward+stemp*playertemp.keystonepassives[34]*(plshp(0.0005)+plsre(0.05))/60);
		});
		append(keystonefuncs.overlay,function(){
			if(render){
				fill(0,0,0);
				rect(825,295,playertemp.keystonepassives[34]*30,10);
	...
	if(playertemp.keystonepassives[27]>0){
		append(keystonefuncs.damagetakenpa,function(){
			pdmg*=1-playertemp.keystonepassives[27]*0.025;
	...
	if(playertemp.keystonepassives[29]>0){
		append(keystonefuncs.damagedealtpostmit,function(){
			if(enemies[index].hp>0){
				if(willhit){
					heal(min(enemies[index].hp,(pdmg+mdmg))*(playertemp.keystonepassives[29]*0.004),"leech");
				}
				else{
					heal(min(enemies[index].hp,(pdmg+mdmg))*(playertemp.keystonepassives[29]*0.004),"LoT");
				}
			}
		});
	}
...
var dodgehit=function(){
	pdmg=0;
	mdmg=0;
	willhit=0;
	willdamage=0;
	append(particles,new createparticle(745,240,-1,random(-0.1,0.1),0,0,'text',"Dodged",30,-0.5,255,-5,200,200,200));
	dmgsound=0;
}
var dmgstashfc=[0,0];
//======================LOAD TRAITS==========================================
var loadtraits=function(){
	...
	append(traitfuncs.damagedealt,function(){
		if(atkprop){
			if(enemies[index].arcaneinstability){
				if(findprop("pierce")||findprop("void")){
					pdmg+=mdmg*0.3;
					armorE*=0.65;
				}
			}
		...
		append(traitfuncs.damagedealt,function(){
			if(willhit){
				if(playertemp.defendcounterattack>0){
					if(pdmg>mdmg){
						pdmg+=plshp(procc*0.15)+plsar(procc*0.15)+plsre(procc*0.12);
					}
					else{
						mdmg+=plshp(procc*0.15)+plsar(procc*0.15)+plsre(procc*0.12);
					}
				}
			}
		});
	...
	if(playertemp.traits[91]>0){
		append(traitfuncs.damagetaken,function(){
			if(playertemp.parrycharge>0){
				if(playertemp.traits[39]>0){
					armorE*=3+playertemp.traits[39]/10;
					resE*=3+playertemp.traits[39]/10;
				}
				else{
					armorE*=3;
					resE*=3;
				}
				if(!(playertemp.parrysustain>0)){
					if(willhit){
						playertemp.parrycharge-=1;
					}
				}
			}
		});
		...
		if(willhit){
			if(enemies[attacker]){
				if(enemies[attacker].unbalancecount){
					if(random(100)<min(80,enemies[attacker].unbalancecount)){
						dodgehit();
					}
				}
			}
	...
	if(playertemp.traits[225]>0){
		append(traitfuncs.damagetaken,function(){
			if(willhit){
				if(random(100)<playertemp.traits[225]){
					dodgehit();
				}
			}
		});
		...
		append(traitfuncs.damagetaken,function(){
			if(willhit){
				if(playertemp.timesincehittaken>600/(1+playertemp.traits[28]/4)){
					...
					append(particles,new createparticle(745,235,-0.5,random(-0.1,0.1),0,0,'text',"Blocked",55,-1,255,-5,170,170,255));
	...
	append(traitfuncs.damagetaken,function(){
		...
		if(playertemp.traits[212]>0){
			pdmg=max(pdmg/10,pdmg*(1-plsre(1)*resE/(plsre(1)*resE+3*pdmg)));
			mdmg=max(mdmg/10,mdmg*(1-plsar(1)*armorE/(plsar(1)*armorE+3*mdmg)));
			if(playertemp.traits[87]>0){
				pdmg=max(pdmg/10,pdmg*(1-(0.04*playertemp.traits[87]-0.02)*plsre(1)*resE/(plsre(1)*resE+3*pdmg)));
				mdmg=max(mdmg/10,mdmg*(1-(0.04*playertemp.traits[87]-0.02)*plsar(1)*armorE/(plsar(1)*armorE+3*mdmg)));
			}
		}
		else{
			pdmg=max(pdmg/10,pdmg*(1-plsar(1)*armorE/(plsar(1)*armorE+3*pdmg)));
			mdmg=max(mdmg/10,mdmg*(1-plsre(1)*resE/(plsre(1)*resE+3*mdmg)));
			if(playertemp.traits[87]>0){
				pdmg=max(pdmg/10,pdmg*(1-(0.04*playertemp.traits[87]-0.02)*plsar(1)*armorE/((0.04*playertemp.traits[87]-0.02)*plsar(1)*armorE+3*pdmg)));
				mdmg=max(mdmg/10,mdmg*(1-(0.04*playertemp.traits[87]-0.02)*plsre(1)*resE/((0.04*playertemp.traits[87]-0.02)*plsre(1)*resE+3*mdmg)));
			}
		}
		if(willhit){
	...
	if(playertemp.traits[227]>0){
		append(traitfuncs.damagetakenpa,function(){
			playertemp.mordekaiserW=min(playertemp.mordekaiserW+(pdmg+mdmg)*0.4,plsin(2)+plshp(1.5));
		});
	}
		...
		if(atkprop){
			if(enemies[index].drenchcount){
				if(findprop("cold")||findprop("electric")){
					pdmg*=1+min(100,enemies[index].drenchcount)/50;
					mdmg*=1+min(100,enemies[index].drenchcount)/50;
				}
			}
			if(enemies[index].shockcount){
				pdmg*=1+min(100,enemies[index].shockcount)*0.0075;
				mdmg*=1+min(100,enemies[index].shockcount)*0.0075;
				if(findprop("water")){
					pdmg*=1+min(100,enemies[index].shockcount)/50;
					mdmg*=1+min(100,enemies[index].shockcount)/50;
				}
			}
			if(enemies[index].unbalancecount){
				if(findprop("wind")||findprop("explosion")){
					pdmg*=1+min(100,enemies[index].unbalancecount)/50;
					mdmg*=1+min(100,enemies[index].unbalancecount)/50;
				}
			}
		}
	});
	append(traitfuncs.damagetaken,function(){
		if(enemies[attacker]){
			if(enemies[attacker].drenchcount){
				pdmg*=max(0.2,1-enemies[attacker].drenchcount/100);
				mdmg*=max(0.2,1-enemies[attacker].drenchcount/100);
			}
		}
	});
	append(traitfuncs.damagedealt,function(){
	...
	if(playertemp.traits[7]>0){
		...
		append(traitfuncs.damagetaken,function(){
			if(player.hp<plshp(0.4)){
				pdmg/=1+playertemp.traits[7]/50;
				mdmg/=1+playertemp.traits[7]/50;
			}
		});
	}
	...
	if(playertemp.traits[9]>0){
		append(traitfuncs.damagedealtpostmit,function(){
			if(enemies[index].hp>0){
				if(willhit){
					heal(min(enemies[index].hp,(pdmg+mdmg))*(playertemp.traits[9]*0.006),"leech");
				}
				else{
					heal(min(enemies[index].hp,(pdmg+mdmg))*(playertemp.traits[9]*0.006),"LoT");
				}
			}
		});
	}
			...
			if(willhit){
				if(findprop("impact")){
					if(enemies[index].isBoss){
						enemies[index].stun+=round(playertemp.traits[222]*50*120*((pdmg+mdmg)/enemies[index].mhp)*(100-enemies[index].tenacity)/100);
					}
					else{
						enemies[index].stun+=round(playertemp.traits[222]*120*((pdmg+mdmg)/enemies[index].mhp)*(100-enemies[index].tenacity)/100);
					}
				}
			}
		});
			...
			if(willhit){
				if(findprop("ironmanshield")){
					if(playertemp.action.name=="mace of spades"){
						...
						playertemp.ironman+=stemp*mdmg/6;
		...
		append(traitfuncs.damagedealtpostmit,function(){
			...
			if(willhit){
				if(findprop("darknessrise")){
					...
					playertemp.mordekaiserW=min(playertemp.mordekaiserW+(pdmg+mdmg)*0.025,plsin(2)+plshp(1.5));
				}
			}
		});
	}
	...
	if(playertemp.traits[91]>0){
		append(traitfuncs.passives,function(){
			...
			if(playertemp.eartharmort>0){
				playertemp.eartharmort-=1;
			}
			else if(playertemp.eartharmor>0){
				playertemp.eartharmor=0;
			}
			if(render){
				translate(400,350);
				rotate(-tick/120);
				if(playertemp.eartharmor>19){
					fill(100,85,0,205+abs(tick%100-50));
				}
				else{
					fill(50,40,20,80+abs(tick%200-100));
				}
				for(stf=0;stf<playertemp.eartharmor;stf+=1){
					rotate(PI*2/playertemp.eartharmor);
					rect(-5,abs(tick%100-50)/10-27.5,5,5);
				}
				resetMatrix();
			}
		...
		if(playertemp.eartharmor>19){
			if(willhit){
				if((pdmg+mdmg)>plshp(0.05)){
					sfx.shattershield.play();
					append(particles,new createparticle(playertemp.x,playertemp.y,0,0,0,0,'circle','',70,1,200,-5,120,100,0,1));
					append(objects,{
						type:'AoE',
						target:'enemy',
						size:50,
						...
						x:playertemp.x,
						y:playertemp.y,
						pdmgmin:plsst(20)+plsar(5)+plshp(2),
						pdmgmax:plsst(24)+plsar(5)+plshp(2),
						mdmgmin:0,
						mdmgmax:0,
						armorE:1,
						resE:1,
	...
	if(playertemp.traits[12]>0){
		...
		append(traitfuncs.passives,function(){
			...
			playertemp.energyshieldmax=((plshp(0.008))+(plsre(0.04))+plsin(0.03))*playertemp.traits[12];
			playertemp.energyshieldrec=((plshr(0.008))+(plsre(0.00025))+plsst(0.002))*playertemp.traits[12]/60;
			stemp=1;
	...
	if(playertemp.traits[226]>0){
		append(traitfuncs.overlay,function(){
			...
			if(playertemp.mordekaiserWactive){
				...
				rect(730,300,30,-(playertemp.mordekaiserWactive/(plsin(2.1)+plshp(1.575))*200));
			}
	...
	if(playertemp.traits[66]>0){
		append(traitfuncs.passives,function(){
			if(getbuffind(9)>=0){
				if(playertemp.buffs[getbuffind(9)].pow>=8){
					playertemp.buffs[getbuffind(9)].pow-=8;
					...
					append(objects,{
						...
						pdmgmin:0,
						pdmgmax:0,
						mdmgmin:((plsin(11)))*(0.6+playertemp.traits[66]*0.4),
						mdmgmax:((plsin(12)))*(0.6+playertemp.traits[66]*0.4),
						armorE:1,
						resE:1,
						procc:0.5*(0.6+playertemp.traits[66]*0.4),
						properties:["arcane"],
						hits:new Array(999),
						onhit:function(){
							if(objects[n].hitc>0){
						...
						endfunc:function(){
							player.mp+=min(5,objects[n].hitc*5);
						}
					});
				}
			}
	...
	if(playertemp.traits[91]>0){
		append(traitfuncs.underlay,function(){
			...
			if(!(playertemp.parrycharge)){
				playertemp.parrycharge=0;
			}
			if(!(playertemp.parrysustain)){
				playertemp.parrysustain=0;
			}
			if(!(playertemp.parrycharget)){
				playertemp.parrycharget=0;
			}
			if(playertemp.parrysustain>0){
				playertemp.parrysustain-=1;
			}
			if(playertemp.parrycharge>0){
				if(playertemp.parrycharget>0){
					playertemp.parrycharget-=1;
				}
				else{
					playertemp.parrycharge=0;
				}
				if(render){translate(400,350);
				rotate(tick%90*PI/45);
				if(playertemp.parrysustain>0){
					fill(200,200,115,255);
				}
				else{
					fill(180,180,115,190);
				}
				for(stf=0;stf<playertemp.parrycharge;stf+=1){
					rotate(PI/5);
					triangle(0,-30,-8,3,8,3);
				}
				resetMatrix();}
			}
		});
...
var heal=function(healp,healtype){
	if(healp>0){
		...
		if(playertemp.traits[212]>0&!(healtype=="static")){
			...
			if(playertemp.keystonepassives[29]>0){
				if(healtype=="leech"){
					playertemp.strfb+=healp*0.0075*playertemp.keystonepassives[29];
					playertemp.intelfb+=healp*0.0075*playertemp.keystonepassives[29];
					append(stateffects,{name:'blood rage',tick:0,bst:healp*0.0075*playertemp.keystonepassives[29],run:function(){
						if(stateffects[n].tick>=300){
			...
			if(playertemp.traits[114]>0){
				if(healtype=="leech"){
					shield(healp*(0.27+playertemp.traits[114]*0.03),300);
					healp*=1-(0.27+playertemp.traits[114]*0.03);
				}
			}
			player.hp+=healp;
...
var damage=function(targetgroup,indexs,pdmgs,mdmgs,armorEs,resEs,attacktypes,attackers,proccs,atkprops){
	...
	if(targetgroup=='player'){
		...
		for(tfdt=0;tfdt<keystonefuncs.dmgpremod.length;tfdt+=1){
			keystonefuncs.dmgpremod[tfdt]();
		}
		...
		if(enemies[attacker]){
			if(enemies[attacker].traits){
				if(enemies[attacker].traits.damagedealtpm){
					for(tdd=0;tdd<enemies[attacker].traits.damagedealtpm.length;tdd+=1){
						enemies[attacker].traits.damagedealtpm[tdd]();
					}
				}
			}
		}
					...
					if(dmgsound){
						if(sfxstock.hurt>0){
							...
							if(armorsfx>0){
								...
								else if(sfxstock.lightarmor>0){
									sfxstock.lightarmor=0;
			...
			//&options.dmgindicators){
				if(pdmg>0){
					if(armorE>0){
						...
						append(particles,new createparticle(765,235,0,-1.5,0,0,'text',"-"+round(pdmg),55,-1,255,-3.5,255,255,255));
					}
				}
				if(mdmg>0){
					if(resE>0){
						...
						append(particles,new createparticle(765,245,0,1.5,0,0,'text',"-"+round(mdmg),55,-1,255,-3.5,255,255,255));
					}
				}
				if(shdmg>0){
	...
	//NME DMG=================
		...
		if((enemies[index].armor*armorE/(enemies[index].armor*armorE+3*pdmg))>0.45&(enemies[index].res*resE/(enemies[index].res*resE+3*mdmg))>0.45){
			armorsfx=1;
			if((enemies[index].armor*armorE/(enemies[index].armor*armorE+3*pdmg))>0.88&(enemies[index].res*resE/(enemies[index].res*resE+3*mdmg))>0.88){
				armorsfx=2;
			}
		}
		pdmg=max(pdmg/10,pdmg*(1-enemies[index].armor*armorE/(enemies[index].armor*armorE+3*pdmg)));
		mdmg=max(mdmg/10,mdmg*(1-enemies[index].res*resE/(enemies[index].res*resE+3*mdmg)));
...
var getindsize=function(val){
	//console.log(7+20*val/(nmelvsc(biomedata[9])*200));
	return(round(min(20,7+20*val/(nmelvsc(biomedata[9])*300))));
}
//////////////////////////////LOAD//////////////////////////////////////=======================================
var loadArea=function(){
	if(player.inrift){
		if(player.riftlvc%25==0){
			player.inrift=0;
			player.biomeID=1;
		}
	}
...
var nmelvsc=function (nmelv){
	...
	return((0.9+nmelv/10)*(1+nmelv/100)*(0.45+(min(100,max(0,nmelv-20))+min(100,max(0,nmelv/2-50)))*0.018)*(1+min(2,max(0,nmelv-10)/600)));
}
var qtipc=false;
			...
			dialog=function(){
				sdialogb({
					...
					answers:[
						...
						{
							answer:"Omnipotency (balanced - recommended if you are new to the game)",
							effect:function(){
								player.ppath=2;
								convergingdialog.introend();
							}
...
var bossthings={
	thatbeast:{
		hascow:function(n){
			for(fc=0;fc<stateffectsg[n].cow.length;fc+=1){
				if(bol==stateffectsg[n].cow[fc]){
					stateffectsg[n].cow[fc]=-1;
					return(true);
				}
			}
			return(false);
...
var enterrift=function(lv){
	...
	if(lv%25==0){
		...
		player.biomeID=2;
		setarena();
		placeboss[round(random(-0.49,placeboss.length-0.51))](lv);
		//placeboss[1](lv);
	}
	else{
		player.biomeID=round(random(1.51,biomemax+0.49));
		loadArea();
	}
}
var setarena=function(){
	clearobjects();
	terrain=new Array(0);
	waters=new Array(0);
	enemies=new Array(0);
...
//0: Shrek
	function(blv){
		player.biomeID=13;
		biomedata[2]=[30,50,0];
		bg=loadImage('Data/Graphics/background/13.png');
		if(options.loadAudio){
			sfx.shrek.play();
			bgm.stop();
		}
		...
		append(enemies,{
			...
			size:22,
			name:"Shrek",
			mhp:12000*nmelvsc(blv),
			hp:12000*nmelvsc(blv),
			aspdm:1,
			boost:1,
			ai:3,
			exists:1,
			...
			speed:2.3,
			str:80*nmelvsc(blv),
			intel:55*nmelvsc(blv),
			armor:40*nmelvsc(blv),
			res:40*nmelvsc(blv),
			dir:0,
			turnstyle:1,
			tenacity:92,
				...
				if(enemies[i].action.id==3){
					if(enemies[i].action.active&!(enemies[i].action.proc)){
						if(enemies[i].y<-375||enemies[i].y>275||enemies[i].x<-375||enemies[i].x>375){
							...
							damage('player',0,random(enemies[i].str*10,enemies[i].str*11),0,1,1,"melee",i,0,["impact"]);
						}
					}
				}
				if(enemies[i].y<-375){
			...
			ondeath:function(i){
				...
				if(player.inrift){
					player.riftlv=max(player.riftlv,biomedata[9]);
				}
				append(stateffectsg,{name:'arena exit',x:0,y:100,tick:0,run:function(){
					if(render){
						fill(0,100,0,150);
						ellipseMode(CENTER);
					...
					//Charge
						...
						if(enemies[i].action.tick<10){
							...
							if(enemies[i].action.active){
								...
								enemies[i].ptenacity=enemies[i].tenacity;
								enemies[i].tenacity=100;
								enemies[i].cds.earthquake-=420;
								damage('player',0,random(enemies[i].str*6,enemies[i].str*7),0,1,1,"melee",i,0,["impact"]);
							}
							if(enemies[i].action.tick>40){
								if(enemies[i].action.active){
									enemies[i].tenacity=enemies[i].ptenacity;
								}
								enemies[i].action.tick=0;
								enemies[i].action.id=1;
							}
...
//1: The Bushes of Love
	function(blv){
		...
		append(stateffectsg,{name:'the bushes of love and also scrap piles',cow:new Array(),beast:round(random(-0.49,18.49)),triggered:0,tick:0,gen:1,run:function(){
			if(stateffectsg[n].gen){
				var gbigc;
				for(bigc=0;bigc<4;bigc+=1){
					gbigc=round(random(-0.49,18.49));
					if(!(gbigc==stateffectsg[n].beast)){
						append(stateffectsg[n].cow,gbigc);
					}
				}
				stateffectsg[n].gen=0;
			}
			...
			for(bol=0;bol<18;bol+=1){
				if(render){
					translate((bol%6)*140+50-playertemp.x,(floor(bol/6))*140-playertemp.y);
					for(dri=0;dri<6;dri+=1){
						for(dr=0;dr<5;dr+=1){
							if(bol%2==0){
								if(stateffectsg[n].triggered){
									fill(0,120,0,255-stateffectsg[n].tick);
								}
								else{
									fill(0,120,0);
								}
								...
								if((dri+dr)%3==0){
									if(stateffectsg[n].triggered){
										fill(50,50,0,255-stateffectsg[n].tick);
									}
									else{
										fill(50,50,0);
									}
									...
									if(stateffectsg[n].triggered){
										fill(50,50,70,255-stateffectsg[n].tick);
									}
									else{
										fill(50,50,70);
									}
									...
									if(stateffectsg[n].triggered){
										fill(50,50,40,255-stateffectsg[n].tick);
									}
									else{
										fill(50,50,40);
									}
							...
							ellipse(dri*10-30+(dr%2)*4,dr*10-25,15,15);
						}
					}
					resetMatrix();
				}
				if((bol%6)*140+50-400>playertemp.x-50&(bol%6)*140+50-400<playertemp.x+50&(floor(bol/6))*140-350>playertemp.y-50&(floor(bol/6))*140-350<playertemp.y+50){
					if(!(stateffectsg[n].triggered)&(bol==stateffectsg[n].beast)){
						stateffectsg[n].tick=0;
						stateffectsg[n].triggered=1;
						append(enemies,{
							lv:blv,
							x:(bol%6)*140+50-400,
							...
							speed:3,
							str:70*nmelvsc(blv),
							intel:85*nmelvsc(blv),
							armor:30*nmelvsc(blv),
							res:50*nmelvsc(blv),
							dir:0,
							turnstyle:1,
							tenacity:92,
							stun:0,
							propertydmods:setcustomprop("earth:0.9/fire:1.1/void:1.05"),
							action:{
								id:0,
								tick:0,
								stock:0
							...
							seeds:20,
							spritec:{
								main:loadShape("Data/Graphics/bosses/thatbeast/body.svg"),
								wing:loadShape("Data/Graphics/bosses/thatbeast/wing.svg"),
							},
							draw:function(i){
								if(enemies[i].y<-375){
									enemies[i].y=-375;
								...
								shape(enemies[i].spritec.main,0,0,60*(1+enemies[i].height/100),(1+enemies[i].height/100)*(90));
								translate(-10*(1+enemies[i].height/100),0);
								rotate(abs(enemies[i].wingtick%18-9)/6);
								shape(enemies[i].spritec.wing,0,0,60*(1+enemies[i].height/100),(1+enemies[i].height/100)*(90));
								rotate(-abs(enemies[i].wingtick%18-9)/6);
								translate(20*(1+(enemies[i].height+6)/100),0);
								rotate(-abs(enemies[i].wingtick%18-9)/6);
								shape(enemies[i].spritec.wing,0,0,-60*(1+enemies[i].height/100),(1+enemies[i].height/100)*(90));
							},
							loot:0,
							...
							ondeath:function(i){
								...
								append(stateffectsg,{name:'arena exit',x:0,y:100,tick:0,run:function(){
									...
									if(tick%45==0&stateffectsg[n].x-playertemp.x<400){
										append(particles,new createparticle(stateffectsg[n].x,stateffectsg[n].y,0,0,0,0,'circle','',25,3,75,-2.5,0,160,0,1));
									}
									if(pow(playertemp.x-stateffectsg[n].x,2)+pow(playertemp.y-stateffectsg[n].y,2)<pow(30+player.size,2)){
										qtipc="Press F to exit";
										if(!(portallock)&keyPressed&(keyCode==UP||key.code==70||key.code==102)){
							...
							aim:function(i){
								if(enemies[i].cds.vinetrap>0){
									enemies[i].cds.vinetrap-=1;
								}
								else{
									if(enemies[i].seeds>0){
										enemies[i].seeds-=1;
								...
								if(enemies[i].cds.fly>0){
									enemies[i].cds.fly-=1;
								}
								//Spawn
								if(enemies[i].action.id==0){
									nmesa.pointatplayer(i);
									enemies[i].wingtick=9+enemies[i].action.tick%3;
									if(enemies[i].cds.height<0){
										enemies[i].action.height+=1;
									}
									else{
										enemies[i].action.tick=0;
										enemies[i].action.stock=0;
										enemies[i].action.id=1;
									}
								}
								//Move
								else if(enemies[i].action.id==1){
									nmesa.pointatplayer(i);
									nmesa.displace(i,1);
									enemies[i].height=0;
									enemies[i].wingtick=9+enemies[i].action.tick%3;
									...
									else if(pow(playertemp.x-enemies[i].x,2)+pow(playertemp.y-enemies[i].y,2)<pow(35,2)){
										enemies[i].action.tick=0;
										enemies[i].action.stock=0;
										enemies[i].action.id=2;
									}
								...
								//Slash
									...
									enemies[i].action.stock+=1+min(180,enemies[i].action.tick)/100;
									if(enemies[i].action.stock>12){
										enemies[i].action.stock-=12;
										if(options.loadAudio){
											sfx.rapier.rate(random(1,1.2));
											sfx.rapier.volume(options.sfx*0.3);
											sfx.rapier.play();
										}
										for(cp=0;cp<10;cp+=1){
											append(particles,new createparticle(enemies[i].x+sin(enemies[i].dir)*35+random(-(40+min(80,enemies[i].action.tick*0.7))/2,(40+min(80,enemies[i].action.tick*0.7))/2),enemies[i].y-cos(enemies[i].dir)*35+random(-(40+min(80,enemies[i].action.tick*0.7))/2,(40+min(80,enemies[i].action.tick*0.7))/2),0,0,0,0,'circle','',15,-0.6,255,-13,random(255),0,0,1));
										}
										append(objects,{
											type:'AoE',
											target:'player',
											size:40+min(80,enemies[i].action.tick*0.7)/2,
											duration:0,
											rangetype:"melee",
											source:i,
											x:enemies[i].x+sin(enemies[i].dir)*35,
											y:enemies[i].y-cos(enemies[i].dir)*35,
											pdmgmin:enemies[i].str*9,
											pdmgmax:enemies[i].str*11,
											hitc:0,
											mdmgmin:0,
											mdmgmax:0,
											armorE:1,
								...
								//Fly
									...
									if(enemies[i].action.tick<50){
										...
										if(enemies[i].action.active){
											playertemp.x=enemies[i].x+sin(enemies[i].dir)*25;
											playertemp.y=enemies[i].y-cos(enemies[i].dir)*25;
											if(enemies[i].action.tick%45==0){
												enemies[i].dir=random(2*PI);
												damage('player',0,random(enemies[i].str*7,enemies[i].str*8),0,1,1,"melee",i,0,["pierce"]);
											}
										}
										else if(pow(playertemp.x-enemies[i].x,2)+pow(playertemp.y-enemies[i].y,2)<pow(20+player.size,2)){
											enemies[i].action.active=1;
											enemies[i].ptenacity=enemies[i].tenacity;
											enemies[i].tenacity=100;
										}
					...
					else if(bossthings.thatbeast.hascow(n)){
						append(enemies,{
							lv:blv,
							x:(bol%6)*140+50-400,
							y:(floor(bol/6))*140-350,
							...
							tenacity:50,
							stun:0,
							propertydmods:setcustomprop("nature:0.85/bleed:1.3/slash:1.1"),
							haste:1,
							walkt:0,
							xp:15,
							ppv:0,
							...
							draw:function(i){
								...
								fill(50,40,0);
								rect(-13,-20,8,-abs(enemies[i].walkt%20-10)*1.5);
								rect(-13,20,8,abs(enemies[i].walkt%20-10)*1.5);
								rect(5,-20,8,-abs((enemies[i].walkt+10)%20-10)*1.5);
								rect(5,20,8,abs((enemies[i].walkt+10)%20-10)*1.5);
								rect(-15,-25,30,50,5);
								rect(-10,-35,20,30,5);
								fill(200,200,200);
...
var getBiomeScripts=function(){
	...
	if(player.biomeID==1){
		...
		append(biomescripts,function(){
			...
			if(!(cinematic||dialoga)){
				if(pow(playertemp.x,2)+pow(playertemp.y+190,2)<pow(100+player.size,2)){
			...
			if(render){
				ellipseMode(CENTER);
				translate(400-playertemp.x,520-playertemp.y);
				noFill();
				stroke(255,255,255);
				strokeWeight(10);
				ellipse(0,0,100,100);
				noStroke();
				fill(100+abs(tick%50-25)*2,100+abs(tick%50-25)*2,100+abs(tick%50-25)*2);
				rotate(tick/100);
				for(dn=0;dn<4;dn+=1){
					rect(0,-10,120,20);
					triangle(60,-20,60,20,80,0);
					rotate(PI/2);
				}
				noFill();
				stroke(abs(tick%100-50)*255/50,abs(tick%124-62)*255/62,abs(tick%142-71)*255/71);
				strokeWeight(1+abs(tick%120-60)/4);
				ellipse(0,0,abs(tick%120-60)*2,abs(tick%120-60)*2);
				resetMatrix();
				noStroke();
			}
			if(playertemp.x<35&playertemp.x>-35&playertemp.y<205&playertemp.y>150){
				playertemp.x-=playertemp.xvelo;
				playertemp.y-=playertemp.yvelo;
				playertemp.xvelo=0;
				playertemp.yvelo=0;
			...
			if(!(cinematic||dialoga)){
				if(playertemp.x<70&playertemp.x>-70&playertemp.y<240&playertemp.y>100){
					qtipc="Press F to use the Space-time Manipulator";
					if(keyPressed&(keyCode==UP||key.code==70||key.code==102)){
						if(player.level<riftstart){
							cinematic[0]=="dialog";
							dialoga=1;
							dialog=function(){
								sdialogb({
									speaker:"Space-time Manipulator",
									speech:"This device is capable of creating rifts within space-time, allowing you to visit areas (even secret ones and places you haven't found) at different difficulties. However, you aren't strong enough to utilize it yet (LV "+player.level+"/"+riftstart+")",
									answers:[
										{
											answer:"(Leave)",
											effect:function(){
							...
							player.riftlv=max(riftstart,player.riftlv);
							inventory=1;
							textinput="";
							inventype=5;
							helpscreen={active:0,help:0};
										...
										function(){
											...
											if(enemies[i].ts.charge>=20){
												...
												append(objects,{
													...
													pdmgmin:0,
													pdmgmax:0,
													mdmgmin:enemies[i].intel*3.2,
													mdmgmax:enemies[i].intel*3.3,
	...
	//Rift portal
	if(player.inrift){
		append(stateffectsg,{name:'rift portal',lv:biomedata[9],x:random(-1200,1200),y:random(-1200,1200),tick:0,run:function(){
			if(render){
				fill(200,255,255);
				textFont(txtfont,16);
				textAlign(CENTER);
				text("Rift Portal",350+stateffectsg[n].x-playertemp.x,285+stateffectsg[n].y-playertemp.y,100,50);
				textAlign(TOP,LEFT);
				fill(110+abs(tick%90-45)*2,110+abs(tick%90-45)*2,140,150);
				ellipseMode(CENTER);
				ellipse(400+stateffectsg[n].x-playertemp.x,350+stateffectsg[n].y-playertemp.y,45,60);
			}
			if(stateffectsg[n].x-playertemp.x<-1300||stateffectsg[n].x-playertemp.x>1300||stateffectsg[n].y-playertemp.y<-1300||stateffectsg[n].y-playertemp.y>1300){
				stateffectsg[n].x=random(-1200,1200)+playertemp.x;
				stateffectsg[n].y=random(-1200,1200)+playertemp.y;
				while(stateffectsg[n].x-playertemp.x<400&stateffectsg[n].x-playertemp.x>-400&stateffectsg[n].y-playertemp.y<350&stateffectsg[n].y-playertemp.y>-350){
					stateffectsg[n].x=random(-1200,1200)+playertemp.x;
			...
			if(tick%20==0&stateffectsg[n].x-playertemp.x<400){
				append(particles,new createparticle(stateffectsg[n].x,stateffectsg[n].y,0,0,0,0,'circle','',25,3.5,100,-4,255,255,255,1));
			}
			if(pow(playertemp.x-stateffectsg[n].x,2)+pow(playertemp.y-stateffectsg[n].y,2)<pow(30+player.size,2)){
				qtipc="Press F to proceed";
				if(!(portallock)&keyPressed&(keyCode==UP||key.code==70||key.code==102)){
					portallock=1;
					enterrift(biomedata[9]+5);
					if(options.autosave){
			...
			if(stateffectsg[n]){
				if(!(stateffectsg[n].lv==biomedata[9])){
					stateffectsg.splice(n,1);
					n-=1;
				}
			}
...
var specialtextcs=[
	...
	function(){
		return(100-100/(1+playertemp.traits[7]/50));
	},
];
...
var nmestatusfx={
	burn:function(power,duration,source){
			if(options.loadAudio&showdots[3]==0){
				sfx.burn.play();
				showdots[3]=1;
			}
			append(dots,{source:source,name:'burn',pow:power,dur:duration,run:function(){
				showdots[0]=1;
				showdots[3]=1;
				if(areatemp.raining>0){
					damage("player",0,0.75*dots[n].pow/3,0.75*dots[n].pow/3,0,0,"DoT",dots[n].source,0,["fire","burn","ignite"]);
					dots[n].dur-=1;
				}
				else{
					damage("player",0,0.75*dots[n].pow/2,0.75*dots[n].pow/2,0,0,"DoT",dots[n].source,0,["fire","burn","ignite"]);
				}
	...
	bleed:function(power,duration,source){
			if(options.loadAudio&showdots[4]==0){
				sfx.bleed.play();
				showdots[4]=1;
			}
			append(dots,{source:source,name:'bleed',pow:power,dur:duration,run:function(){
				showdots[1]=1;
				showdots[4]=1;
				damage("player",0,0.75*dots[n].pow,0,0.018,0.018,"DoT",dots[n].source,0,["bleed"]);
				if(dots[n].dur<=0){
					dots.splice(n,1);
					n-=1;
				}
	...
	poison:function(power,duration,source){
		if(options.loadAudio){
			if(showdots[5]==0){
				sfx.poisoned.play();
				showdots[5]=1;
		...
		append(dots,{source:source,name:'poison',pow:power,dur:duration,run:function(){
			showdots[2]=1;
			showdots[5]=1;
			damage("player",0,0,0.75*dots[n].pow,0.018,0.018,"DoT",dots[n].source,0,["poison"]);
			if(dots[n].dur<=0){
				dots.splice(n,1);
				n-=1;
			}
...
var nmeonhit=function(effect,chance,power,duration){
	if(chance>=random(0,100)){
		if(effect==1){
			nmestatusfx.burn(power,duration,i);
		}
		if(effect==2){
			nmestatusfx.bleed(power,duration,i);
		}
		if(effect==3){
			nmestatusfx.poison(power,duration,i);
		}
	}
}
...
var statpanel=function(){
		...
		if(cursorbox(20,300,305,330)){
			fafgc=["",0,""];
			if(playertemp.traits[217]>0){
				fafgc[1]=round((((1+playertemp.traits[217]/100)*(plsar(1)))/(((1+playertemp.traits[217]/100)*(plsar(1)))+nmelvsc(player.level)*39))*1000)/10;
			}
			else{
				fafgc[1]=round((((plsar(1)))/(((plsar(1)))+nmelvsc(player.level)*39))*1000)/10;
			}
			if(playertemp.traits[212]>0){
				fafgc[0]="magic";
			}
			...
			if(playertemp.traits[87]>0){
				if(playertemp.traits[217]>0){
					fafgc[2]="Estimated Nullification: "+min(90,round((((1+playertemp.traits[217]/100)*(0.04*playertemp.traits[87]-0.02)*(plsar(1)))/(((1+playertemp.traits[217]/100)*(0.04*playertemp.traits[87]-0.02)*(plsar(1)))+nmelvsc(player.level)*39))*1000)/10)+"%";
				}
				else{
					fafgc[2]="Estimated Nullification: "+min(90,round((((0.04*playertemp.traits[87]-0.02)*(plsar(1)))/(((0.04*playertemp.traits[87]-0.02)*(plsar(1)))+nmelvsc(player.level)*39))*1000)/10)+"%";
				}
			}
			tooltipdraw={
				type:1,
				...
				"Estimated reduction: "+min(90,fafgc[1])+"% (vs "+round(nmelvsc(player.level)*13)+" damage).",
				"(caps at 90% reduction.)",
				"Formula: armor/(armor+damage*3)",
				fafgc[2],
				"Armor determined by:",
		...
		if(cursorbox(20,300,335,360)){
			fafgc=["",0,""];
			if(playertemp.traits[218]>0){
				fafgc[1]=round((((1+playertemp.traits[218]/100)*(plsre(1)))/(((1+playertemp.traits[218]/100)*(plsre(1)))+nmelvsc(player.level)*39))*1000)/10;
			}
			else{
				fafgc[1]=round((((plsre(1)))/(((plsre(1)))+nmelvsc(player.level)*39))*1000)/10;
			}
			if(playertemp.traits[212]>0){
				fafgc[0]="physical";
			}
			...
			if(playertemp.traits[87]>0){
				if(playertemp.traits[218]>0){
					fafgc[2]="Estimated Nullification: "+min(90,round((((1+playertemp.traits[218]/100)*(0.04*playertemp.traits[87]-0.02)*(plsre(1)))/(((1+playertemp.traits[218]/100)*(0.04*playertemp.traits[87]-0.02)*(plsre(1)))+nmelvsc(player.level)*39))*1000)/10)+"%";
				}
				else{
					fafgc[2]="Estimated Nullification: "+min(90,round((((0.04*playertemp.traits[87]-0.02)*(plsre(1)))/(((0.04*playertemp.traits[87]-0.02)*(plsre(1)))+nmelvsc(player.level)*39))*1000)/10)+"%";
				}
			}
			tooltipdraw={
				type:1,
				...
				"Estimated reduction: "+min(90,fafgc[1])+"% (vs "+round(nmelvsc(player.level)*13)+" damage).",
				"(caps at 90% reduction.)",
				"Formula: resistance/(resistance+damage*3)",
				fafgc[2],
...
var ts={
	...
	explosivemunitions:function(tpm){
			if(playertemp.traits[65]/10>=random(1)){
				sfx.bomb.play();
				append(particles,new createparticle(objects[n].x,objects[n].y,0,0,0,0,'circle','',50,3,255,-13,180,150,0,1));
				append(objects,{
					type:'AoE',
					target:'enemy',
					size:60,
	...
	if(stancecache[hand].current==0){
		...
		playertemp.action.run=function(){
			...
			if(playertemp.action.tick<=30){
				...
				resetMatrix();
				if(playertemp.action.empowered==2){
					append(particles,new createparticle(400+sin(playertemp.action.dir)*random(10,45),350-cos(playertemp.action.dir)*random(10,45),0,0,0,0,'circle','',6,0,255,-20,255,255,random(160,255)));
				}
				else if(playertemp.action.empowered==3){
					append(particles,new createparticle(400+sin(playertemp.action.dir)*random(10,45),350-cos(playertemp.action.dir)*random(10,45),0,0,0,0,'circle','',6,0,255,-20,255,120,random(60,120)));
				}
				else{
					append(particles,new createparticle(400+sin(playertemp.action.dir)*random(10,45),350-cos(playertemp.action.dir)*random(10,45),0,0,0,0,'circle','',6,0,200,-20,180,180,random(120,180)));
				}
			}
			if(playertemp.action.swingdir){
			...
			if(playertemp.action.tick==1){
				if(options.loadAudio){
					if(playertemp.bswordc>2){
						sfx.pswing.rate(random(0.9,1.1));
						sfx.pswing.play();
					}
					else{
				...
				for(i=0;i<enemies.length;i+=1){
					if(!(enemies[i].invinci>0)){
						...
						(enemies[i].x-playertemp.x)*sin(-playertemp.action.diro)+(enemies[i].y-playertemp.y)*cos(-playertemp.action.diro)<0
							...
							if(playertemp.bswordc==3){
								if(enemies[i].isBoss){
									damage("enemies",i,random(20*(plsst(1)),22*(plsst(1)))+max(0,min((plsst(25))+(plshp(1)),(enemies[i].mhp*1.2-enemies[i].hp)*6)),0,1,1,"melee","player",1,["slash"]);
								}
								else{
									damage("enemies",i,random(20*(plsst(1)),22*(plsst(1)))+max(0,min((plsst(100))+(plshp(4)),(enemies[i].mhp*1.2-enemies[i].hp)*6)),0,1,1,"melee","player",1,["slash"]);
								}
							}
							else{
		...
		if(playertemp.bswordc>3){
			...
			playertemp.action.run=function(){
				...
				if(playertemp.action.tick<15){
					...
					ellipse(400+sin(playertemp.action.dir)*25,350-cos(playertemp.action.dir)*25,30,30);
					append(particles,new createparticle(400+sin(playertemp.action.dir)*random(10,70),350-cos(playertemp.action.dir)*random(10,70),0,0,0,0,'circle','',10,0,200,-20,180,120,random(0,50)));
				}
				else if(playertemp.action.tick<25){
					playertemp.action.speedm=0.3;
					fill(170,110,95);
					ellipse(400+sin(playertemp.action.dir)*25,350-cos(playertemp.action.dir)*25,30-max(0,playertemp.action.tick-15)*1.5,30-max(0,playertemp.action.tick-15)*2);
				}
				else{
					playertemp.action.speedm=0;
					fill(170,110,95,255-max(0,playertemp.action.tick-35)*20);
					ellipse(400+sin(playertemp.action.dir)*25,350-cos(playertemp.action.dir)*25,10,10);
				}
				if(playertemp.action.tick==25){
					playertemp.xvelo=0;
					playertemp.yvelo=0;
					sfx.fissure.play();
					append(particles,new createparticle(400+sin(playertemp.action.dir)*30,350-cos(playertemp.action.dir)*30,0,0,0,0,'circle','',70,0,255,-8,255,200,0));
					append(particles,new createparticle(400+sin(playertemp.action.dir)*30,350-cos(playertemp.action.dir)*30,0,0,0,0,'circle','',0,20,255,-24,255,255,120));
					for(i=0;i<enemies.length;i+=1){
						if(pow(enemies[i].x-playertemp.x+cos(playertemp.action.diro+PI/2)*30,2)+pow(enemies[i].y-playertemp.y+sin(playertemp.action.diro+PI/2)*30,2)<pow(45*(1+traitpow/10)+enemies[i].size,2)){
							damage("enemies",i,random((plsst(36)),(plsst(40))),0,1,1,"melee","player",1.7,["earth","explosion"]);
						}
					}
				append(objects,{
					type:'projectile',
					...
					hits:new Array(999),
					run:function(){
						for(jrt=0;jrt<2;jrt+=1){
							append(particles,new createparticle(objects[n].x+random(-25,25),objects[n].y+random(-25,25),random(-0.25,0.25),random(-0.25,0.25),0,0,'circle','',random(8,15),-0.2,255,-1.25,100+random(30),80+random(40),random(20),1));
						}
					},
					onhit:function(i){
						applyunbalance(i,30,180);
						append(particles,new createparticle(enemies[i].x,enemies[i].y,0,0,0,0,'circle','',30,3,255,-13,120,100,60,1));
						append(objects,{
							type:'AoE',
							target:'enemy',
							size:45*(1+0.07*objects[n].traitpow),
							duration:0,
							rangetype:"melee",
							x:enemies[i].x,
							y:enemies[i].y,
			...
			playertemp.action.run=function(){
				...
				if(playertemp.action.tick<=30){
					...
					if(render){shape(sprites.sword,0,0,30,45);}
					resetMatrix();
					if(playertemp.action.empowered==3){
						append(particles,new createparticle(400+sin(playertemp.action.dir)*random(10,45),350-cos(playertemp.action.dir)*random(10,45),0,0,0,0,'circle','',6,0,200,-20,160,100,random(0,60)));
					}
					else{
						append(particles,new createparticle(400+sin(playertemp.action.dir)*random(10,45),350-cos(playertemp.action.dir)*random(10,45),0,0,0,0,'circle','',6,0,255,-20,110,70,random(0,40)));
					}
				}
				if(playertemp.action.swingdir){
					playertemp.action.dir+=0.12;
				...
				if(playertemp.action.tick==1){
					if(options.loadAudio){
						if(playertemp.bswordc>2){
							sfx.pswing.rate(random(0.8,1));
							sfx.pswing.play();
						}
						else{
							sfx.swing.rate(random(0.8,1));
							sfx.swing.play();
						}
					}
				}
					...
					for(i=0;i<enemies.length;i+=1){
						if(!(enemies[i].invinci>0)){
							...
							(enemies[i].x-playertemp.x)*sin(-playertemp.action.diro)+(enemies[i].y-playertemp.y)*cos(-playertemp.action.diro)<0
								...
								hits+=1;
								damage("enemies",i,random(20*(plsst(1)),22*(plsst(1))),0,1,1,"melee","player",1,["earth","slash"]);
							}
						}
					}
					if(hits>=1){
						playertemp.eartharmort=1800;
	...
	playertemp.action.run=function(){
		...
		if(playertemp.action.tick==8){
			append(objects,{
				...
				x:playertemp.x,
				y:playertemp.y,
				shieldval:((plsar(0.08))+(plshp(0.24)+(plsre(0.04)))*(1+traitpow/20)),
				gmrec:(1+traitpow/20)/2,
					...
					append(objects,{
						...
						run:function(){
							...
							if(pow(playertemp.x-objects[n].x,2)+pow(playertemp.y-objects[n].y,2)<pow(10+player.size+objects[n].speed*2,2)){
								...
								playertemp.guard=min((plshp(1))*(playertemp.traits[91]/100)+playertemp.guardmb,playertemp.guard+((plshp(1))*(playertemp.traits[91]/100)+playertemp.guardmb)*objects[n].gmrec);
								for(crtds=1;crtds<13;crtds+=1){
									shield(objects[n].shieldval/12,180*crtds/12);
								}
								buff(2,300,20);
							}
							}
				...
				if(playertemp.action.tick>=30){
					for(i=0;i<enemies.length;i+=1){
						if(pow(enemies[i].x-playertemp.x,2)+pow(enemies[i].y-playertemp.y,2)<pow(140+enemies[i].size,2)){
							...
							applydrench(i,15,min(200,playertemp.action.tick)*1.5);
							hits+=1;
						}
					}
		...
		if(playertemp.action.name=='rapier'){
			...
			if(playertemp.action.tick==0){
				...
				if(options.loadAudio){
					...
					sfx.rapier.volume(options.sfx*0.5);
					sfx.rapier.play();
				}
			}
			if(playertemp.action.tick<=8){
			...
			if(playertemp.action.tick>=20){
				if(player.mp>=1&mousePressed&((mouseButton==LEFT&playertemp.action.mb==0)||(mouseButton==RIGHT&playertemp.action.mb==1))){
					player.mp-=1;
						...
						playertemp.action.speedm=1;
						spendmana("melee",3,3);
					if(player.mp>=1&playertemp.action.empowered){
						player.mp-=1;
						playertemp.action.speedm=0;
						playertemp.action.hits=new Array(999);
						playertemp.action.hitc=0;
						playertemp.action.name="rapier lunge";
						spendmana("melee",1,1);
					}
				}
			...
			for(i=0;i<enemies.length;i+=1){
				if(!(playertemp.action.hits[i])){
					if(!(enemies[i].invinci>0)){
						if(pow(enemies[i].x-playertemp.x,2)+pow(enemies[i].y-playertemp.y,2)<pow(40+enemies[i].size,2)&
...
append(doaction,function(lv,hand){
	if(player.mp>=20*player.rcostm[0]*playertemp.rcostm[0]){
		...
		heal(plsin(0.08)*(4+playertemp.traits[63]),"direct");
		...
		if(playertemp.action.tick<=5){
			...
			if(playertemp.action.tick==18){
				...
				if(hits>0){
					...
					playertemp.ironman+=(hits+2)*((plsin(1))*2+(plshp(1)))*(0.2+0.1*playertemp.traits[73])*(0.1);
					playertemp.ironmandur=240;
					if(playertemp.ironman>((plsin(1))*2+(plshp(1)))*(0.2+0.1*playertemp.traits[73])){
						playertemp.ironman=((plsin(1))*2+(plshp(1)))*(0.2+0.1*playertemp.traits[73]);	
					}
				}
			}
		}
...
append(doaction,function(lv,hand){
	...
	append(objects,{
		...
		pdmgmin:0,
		pdmgmax:0,
		mdmgmin:player.mp*0.003*(plsst(40)+plsin(60)),
		mdmgmax:player.mp*0.003*(plsst(40)+plsin(60)),
		armorE:1,
		resE:1,
		procc:0,
		hitc:0,
		...
		if(playertemp.action.charge>=5){
			...
			append(objects,{
				...
				onhit:function(i){
					...
					applyarcaneinstability(i,1,120);
				},
				run:function(){
					objects[n].size+=0.4;
					if(!(objects[n].phase)){
						...
						if(objects[n].countstl){
							...
							if(objects[n].prime>=30){
								...
								objects[n].onhit=function(i){
									...
									objects[n].pdmgmin*=0.75;
									objects[n].pdmgmax*=0.75;
									objects[n].mdmgmin*=0.75;
									objects[n].mdmgmax*=0.75;
								}
	...
	if(stancecache[hand].current==0){
		...
		playertemp.action.run=function(){
			...
			if(playertemp.action.ammo>=6){
				...
				append(objects,{
					...
					draw:function(){
						...
						if(options.light){
							...
							for(cal=0;cal<15;cal+=1){
								ellipse(0,4,cal*2*(2.7-objects[n].duration*0.027),cal*4*(2.7-objects[n].duration*0.027));
							}
						}
					},
					target:'enemy',
					...
					hits:new Array(999),
					run:function(){
						objects[n].speed+=0.015;
						objects[n].procc+=0.0014;
						objects[n].mdmgmin+=objects[n].dmggain;
						objects[n].mdmgmax+=objects[n].dmggain;
						if(objects[n].duration==10){
							objects[n].speed+=25;
							objects[n].dir=objects[n].diro;
							objects[n].pdmgmin+=objects[n].sdmg.pdi;
							objects[n].pdmgmax+=objects[n].sdmg.pda;
							objects[n].mdmgmin+=objects[n].sdmg.mdi;
							objects[n].mdmgmax+=objects[n].sdmg.mda;
							objects[n].pdmgmin*=1.5;
							objects[n].pdmgmax*=1.5;
							objects[n].mdmgmin*=1.5;
							objects[n].mdmgmax*=1.5;
							objects[n].fast=3;
							objects[n].empowered=1;
							objects[n].ds=2;
					...
					onhit:function(i){
						...
						if(!(objects[n].empowered)){
							objects[n].sdmg.pdi+=objects[n].pdmgmin*0.3;
							objects[n].pdmgmin*=0.7;
							objects[n].sdmg.pda+=objects[n].pdmgmax*0.3;
							objects[n].pdmgmax*=0.7;
							objects[n].sdmg.mdi+=objects[n].mdmgmin*0.3;
							objects[n].mdmgmin*=0.7;
							objects[n].sdmg.mda+=objects[n].mdmgmax*0.3;
							objects[n].mdmgmax*=0.7;
						}
						append(stateffects,{name:'chill',target:i,tick:0,run:function(){
							if(enemies[stateffects[n].target]){
								if(playertemp.traits[81]>0){
									enemies[i].spdmod=min(enemies[stateffects[n].target].spdmod,0.56);
								}
								else{
									enemies[i].spdmod=min(enemies[stateffects[n].target].spdmod,0.6);
								}
								if((tick)%10==0){
									append(particles,new createparticle(enemies[i].x+random(-enemies[i].size*1.5,enemies[i].size*1.5),enemies[i].y+random(-enemies[i].size*1.5,enemies[i].size*1.5),0,0,0,0,'circle','',12,-0.1,255,-6,155+random(100),155+random(100),255,1));
								}
								if(stateffects[n].tick>=45*(1-enemies[stateffects[n].target].tenacity/100)){
									stateffects.splice(n,1);
									n-=1;
								}
							}
		...
		playertemp.action.run=function(){
			...
			if(playertemp.action.flare>=playertemp.action.req){
				...
				append(stateffects,{name:'surge flare',tick:0,run:function(){
					if(stateffects[n].tick<15){
						stroke(255,255,100,150-stateffects[n].tick*10);
						noFill();
						strokeWeight(15+stateffects[n].tick);
						ellipseMode(CENTER);
						ellipse(400,350,10+stateffects[n].tick*3,10+stateffects[n].tick*3);
						noStroke();
					}
					if(stateffects[n].tick>=15){
						stateffects.splice(n,1);
				...
				for(i=0;i<enemies.length;i+=1){
					if(pow(enemies[i].x-playertemp.x,2)+pow(enemies[i].y-playertemp.y,2)<pow(55+enemies[i].size,2)){
						applyshock(i,10,30+playertemp.action.req);
					}
				}
			}
...
append(doaction,function(lv,hand){
	...
	if(playertemp.mordekaiserWactive<=0){
		...
		playertemp.mordekaiserWactive=playertemp.mordekaiserW+plsin(0.1)+plshp(0.075);
...
if(tick%30==0){
	if(round(healind)>0){
		...
		append(particles,new createparticle(720,healyr,1.5,(250-healyr)/10,0,-(250-healyr)/500,'text',"+"+round(healind),min(35,15+round(healind/plshp(0.02))),0,255,-4,0,255,0));
		healind=0;
	}
}
	if(!(cinematic||dialoga)){
	...
	if(playertemp.stun>0){
		...
		if(playertemp.traits[224]>0){
			playertemp.stunres+=playertemp.traits[224]/10;
		}
	}
	else{
				...
				if(render){
					if(!(enemies[i].stealth>0)){
						...
						if(enemies[i].prefixcolor){
							fill(enemies[i].prefixcolor[0],enemies[i].prefixcolor[1],enemies[i].prefixcolor[2],30+abs(tick%60-30)*3);
							ellipse(0,0,enemies[i].size*2.5+9,enemies[i].size*2.5+9);
						}
						if(enemies[i].suffixcolor){
							fill(enemies[i].suffixcolor[0],enemies[i].suffixcolor[1],enemies[i].suffixcolor[2],30+abs((tick+30)%60-30)*3);
							ellipse(0,0,enemies[i].size*2.5+9,enemies[i].size*2.5+9);
						}
		...
		if(player.hp<=0){
			...
			if(playertemp.inBossFight||!(playertemp.traits[75]>0)){
				if(playertemp.traits[62]>0){
					stemp=3*(1+playertemp.traits[62]/5);
				}
				else{
					stemp=3;
				}
	...
	if(playertemp.traits[211]>0){
		fill(120,120,120,200);
		rect(1125-playertemp.traits[211]*3,335,playertemp.traits[211]*3,30);
	}
	fill(0,0,0);
	rect(825,410,300,15);
	fill(0,80,0);
	...
	textFont(txtfont,15);
	if(player.inrift){
		text("Space-time rift (LV"+biomedata[9]+")",900,675);
	}
	else{
	...
	if(inventory==1){
				...
				if(cursorbox(650+30*n,675+30*n,125+30*x,150+30*x)){
					...
					if(!(mouselock)&mousePressed){
						...
						if(mouseButton==LEFT){
							...
							if(player.inventory.bag[15*x+n]){
								if(playertemp.inBossFight){
									append(particles,new createparticle(mouseX,mouseY-30,0,0,0,0,'text','You cannot equip items in boss arenas!',20,0,255,-3,255,0,0));
								}
								else{
									if(player.inventory.bag[15*x+n].level<=player.level){
										if(options.loadAudio){sfx.click3.play();}
		...
		if(!(player.biomeID==1)&!(mouselock)&mousePressed&cursorbox(400,600,605,685)){
			...
			//Space-time Manipulator
				...
				fill(0,0,abs(tick%120-60));
				rect(0,0,1133,700);
				fill(255,255,0);
				textFont(txtfont,35);
				text("Space-time Manipulator",350,30);
				ellipseMode(CENTER);
				fill(150,150,255);
				stroke(255,255,150,100+abs(tick%120-60));
				strokeWeight(2+abs(tick%150-75)/3);
				ellipse(250,350,150,300);
				fill(255,150,200);
				ellipse(850,350,150,300);
				noStroke();
				textAlign(CENTER);
				fill(0,0,0);
				textFont(txtfont,25);
				if(cursorbox(175,325,200,500)){
					text("Last cleared: LV"+player.riftlv+". No entry fee.",175,275,150,300);
					if(mousePressed){
						enterrift(player.riftlv);
						inventory=0;
						helpscreen={active:0,help:0};
						if(options.loadAudio){
							if(player.hp<(plshp(1))){
				...
				textFont(txtfont,27);
				if(textinput==""){
					text("Previous rift - type a number",200,600,200,100);
				}
				else{
					text("Previous rift - LV"+textinput,200,600,200,100);
				}
				fill(0,255,0);
				rect(550,595,150,50,5);
				fill(255,0,255);
				textFont(txtfont,34);
				text("Enter",570,607,100,50);
				if(cursorbox(550,700,595,645)){
					if(mousePressed){
						textinput=max(5,min(player.riftlv,round(Number(textinput)/5)*5));
						enterrift(textinput);
						inventory=0;
						helpscreen={active:0,help:0};
						if(options.loadAudio){
...
void keyPressed(){
	...
	else if(inventory&inventype==5){
		if(key.code==8||key.code==127){
			textinput=textinput.substring(0,textinput.length-1);
		}
		else if(ascii[key.code]){
			if(key.code>47&key.code<58&textinput.length<20){
				textinput+=ascii[key.code];
			}
		}
	}
