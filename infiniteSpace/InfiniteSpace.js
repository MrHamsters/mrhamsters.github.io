...
var playerdraw=[
	...
	function(){
		if(playertemp.phasing>0){
			ellipseMode(CENTER);
			fill(40,110+abs(tick%40-20)*2,150,20+abs(tick%16-8)*20);
			ellipse(player.x,player.y,60,100);
		}
		else{
...
var applymods=function(){
	...
	if(player.mods[1]){
		...
		append(player.modfuncs.passive,function(){
			player.energy=min(player.menergy,player.energy+0.001+player.menergy*0.00005);
		});
	...
	if(player.mods[10]){
		...
		append(player.modfuncs.damagetaken,function(){
			player.energy=max(0,player.energy-(player.energy/player.menergy)*dmg.dmg/50);
		});
		append(player.modfuncs.damagetakenps,function(){
			player.energy=max(0,player.energy-(player.energy/player.menergy)*dmg.dmg/25);
		});
	}
	if(player.mods[11]){
		append(player.modfuncs.damagedealt,function(edmg,target){
	...
	if(player.mods[15]){
		append(player.modfuncs.passive,function(){
			if(player.wither>0){
				if(gametick%30==0){
					takedamage({dmg:min(1,player.wither)});
...
var mods=[
	...
	{name:"Energized Feedback",desc:"Absorbs the entropy created from destruction",pro:"Gain energy when you deal damage, doubled for normal shots",con:"Backfires when you take damage, losing energy - reduced by 67% for damage blocked by shielding"},
		...
		if(player.stun>0){
			...
			while(engineparticles>0){
				...
				if(player.mods[17]){
					...
					size:random(3,5),op:random(120,180),opc:-4,exp:1,color:[random(60,105)+playertemp.slow*150,random(140,190),random(160,240)-playertemp.slow*150]});
				}
				else{
					append(particles,{x:player.x,y:player.y+player.size*1.5,xvelo:random(-1,1)-sin(moveinf.dir)*moveinf.scl*max(player.staticspeed,player.speed),yvelo:random(-1,3)+cos(moveinf.dir)*moveinf.scl*max(player.staticspeed,player.speed),
